...
esp_err_t Shtc3::read(SensorData& data)
{
    ...
    ESP_RETURN_ON_FALSE(initialized_ && dev_handle_, ESP_ERR_INVALID_STATE, TAG, "SHTC3 not initialized");
    ...
    vTaskDelay(pdMS_TO_TICKS(measurement_mode_ == MeasurementMode::NORMAL_MODE ? SHTC3_MEASURE_DELAY_NORMAL_MS
                                                                               : SHTC3_MEASURE_DELAY_LOW_POWER_MS));
